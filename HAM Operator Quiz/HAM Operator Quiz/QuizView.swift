...
struct QuizView: View {
    ...
    @State private var autoAdvanceWorkItem: DispatchWorkItem? = nil
    ...
    func bubbleColor(for answer: String) -> Color {
        ...
        if let selected = selectedAnswer {
            if selected == answer && selected == correctAnswer {
                ...
                // Selected wrong answer -> red bubble
                return .red
            } else if answer == correctAnswer && selected != correctAnswer {
                // Highlight correct answer bubble green if selected answer is wrong
                return .green
            }
        }
        return imessageBlue
    ...
    func showFinalResultsAndExit() {
        let percentage = currentIndex > 0 ? Int((Double(score) / Double(currentIndex + 1)) * 100) : 0
        feedbackText = "You answered \(score) out of \(currentIndex + 1) correctly. (\(percentage)% correct)"
    ...
    func scheduleAutoNext() {
        cancelAutoNext() // Cancel any existing auto advance
        autoAdvanceWorkItem = DispatchWorkItem {
            if selectedAnswer != nil && currentIndex < questions.count - 1 {
                nextQuestion()
            }
        }
        if let workItem = autoAdvanceWorkItem {
